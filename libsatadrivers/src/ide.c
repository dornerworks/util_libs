 ...
 *
 * A lot of this code was based on code found at http://wiki.osdev.org/PCI_IDE_Controller
...
uint8_t ide_print_error(uint32_t drive, uint8_t err)
{
    ...
    ZF_LOGV("- [%s %s] %s\n",
            (const char *[]){"Primary", "Secondary"}[sata_devices[drive].Channel],
            (const char *[]){"Master", "Slave"}[sata_devices[drive].Drive],
    ...
    for (i = 0; i < NUM_IDE_CHANNELS; i++)
    {
        if (1 == sata_devices[i].Reserved)
        {
            ZF_LOGV(" (%d) Found %s Drive %dGB - %s\n", i,
                    (const char *[]){"ATA", "ATAPI"}[sata_devices[i].Type],  /* Type */
