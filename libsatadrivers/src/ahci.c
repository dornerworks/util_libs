...
#define GET_LOWER_32BITS(val) (uint32_t)((uint64_t)(val) & 0xFFFFFFFFLL)
#define GET_UPPER_32BITS(val) (uint32_t)(((uint64_t)(val) & 0xFFFFFFFF00000000LL) >> 32)
    ...
    for(i = 0; i < NUM_MAX_PORTS; i++) {
        if (pi & 1) {
            ...
            if ((DEV_NULL < dt) && (DEV_NUM_DRIVE_TYPES >= dt)) {
                ZF_LOGV("%s drive found at port %d\n", drive_type_str[dt - 1], i);
                device_list[active_count].port_num = i;
                active_count++;
            }
            else {
                ZF_LOGV("No drive found at port %d\n", i);
            }
        }
