...
int sata_get_partition_tables(uint8_t drive, partition_table_t *partition_tables, uint8_t *part_data) {
    ...
    ZF_LOGV("SATA: Reading partition Tables for Drive: %u\n", drive);
    ...
    /* Check for errors */
    if(err != 0) {
        ZF_LOGE("SATA: ERROR: %d\n", err);
        return err;
    }

    /* Get & Print Partition information from each drive */
    for(int i = 0; i < MAX_DRIVES; i++) {
        memcpy(&partition_tables[i], &part_data[PART_OFFSET+(PART_SIZE*i)], sizeof(partition_table_t));

        ZF_LOGV("Partition %i:\n", i);
        ZF_LOGV("\tStart: %u, Sectors: %u\n", partition_tables[i].start_lba,
                partition_tables[i].num_sectors);
        ZF_LOGV("\thead: %u, end_head: %u\n", partition_tables[i].head,
                partition_tables[i].end_head);
        ZF_LOGV("\tsec_cyl: %u, end_sec_cyl: %u\n", partition_tables[i].sec_cyl,
                partition_tables[i].end_sec_cyl);
        ZF_LOGV("\tBoot: %u, System ID: 0x%X\n", partition_tables[i].boot,
